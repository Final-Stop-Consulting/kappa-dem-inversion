...
1. **Diagnostic degeneracy at the AIA-imaging level.** By the convergence principle of [Edmonds 2026b, Open Transport](https://doi.org/10.1515/ot-2026-0011), every electron-temperature diagnostic mediated by collisional ionization equilibrium structurally returns the effective temperature *T*<sub>eff</sub> of the underlying distribution and cannot detect departure from Maxwellian form. We verify this empirically: a single isothermal κ = 2.5 source returns a multi-thermal-looking DEM at χ²/dof = 1.00 with FWHM = 0.222 in log *T*, just below the narrow edge of the FWHM distribution returned by the same pipeline from real quiet-Sun AIA observations (median 0.283, range 0.230–0.401 across 80 patches at solar minimum; cross-date stable to −0.0025 dex). A multi-thermal κ source (Brooks 2009 shape × κ = 2.5 ion fractions) recovers FWHM = 0.305, comfortably inside the real-QS distribution near its median. The AIA pipeline does not separate single-T κ, multi-T Maxwellian, or multi-T κ on FWHM alone.

2. **Failure of the conductive closure.** The Spitzer–Härm conductivity used in the standard quiet-Sun budget evaluates a Maxwellian moment closure at the EUV-derived *T*<sub>eff</sub>, a tail-weighted moment of the κ distribution. For a κ ≈ 2.5 plasma there is no local Spitzer–Härm conductivity to evaluate at *any* temperature: conduction is the third velocity moment, carried by the suprathermal tail, and the local conductivity integral does not converge across the κ ∈ [2, 3] range. The closed-form κ-conductivity (Du 2013; Husidic et al. 2021; Guo & Du 2019) carries poles at κ = 3 and κ = 4, and the finite −16.5 *λ*<sub>M</sub> it returns at κ = 2.5 is an analytic continuation of a divergent integral, not a physical conductivity (`kappa_conductivity_collapse.py`, machine-precision validation gate against Du Eq. 26). The standard fluid-conductive energy budget therefore rests on a closure with no convergent form in this regime; the conductive flux is a non-local, fully kinetic quantity, and whether the measured Alfvén-wave flux (0.5–5 × 10<sup>5</sup> erg cm<sup>−2</sup> s<sup>−1</sup>) suffices is a question about that total kinetic budget, not a fluid-conductive gap. The result is conditional: the κ ≈ 2.5 premise comes from Edmonds 2026a, and the consequent holds for any plasma in that class, with the quiet Sun as the worked example.

The κ ≈ 2.5 premise comes from [Edmonds 2026a, Open Journal of Astrophysics](https://doi.org/10.33232/001c.161223) (radio–EUV–density triple intersection); the closure-failure result in §4.5 is the quiet-Sun counterpart to the scrape-off-layer analysis of Edmonds 2026b §4.2.5.
...
| Conductive-closure non-existence (Du Eq. 26 poles; integrand divergence) | §4.5.2, Fig. 9 | `kappa_conductivity_collapse.py` | `Results/kappa_conductivity_collapse.{json,txt}` | numpy + scipy | ~5 s |
...
- **Convergence-principle verification.** Three source families (single-T κ, multi-T Maxwellian, multi-T κ) all land in the same recovered-FWHM band as the real-QS pipeline-output distribution. The κ correction averages out under multi-T integration because each AIA channel samples its dominant ion near formation temperature, where the κ/Maxwellian ion-fraction ratio is close to unity.
...
- **Conductive-closure non-existence.** The local Spitzer–Härm conductivity has no convergent form across the κ ∈ [2, 3] range: the standard-κ closed forms (Du 2013; Husidic et al. 2021; Guo & Du 2019) carry poles at κ = 3 and κ = 4, and the only finite (regularized-κ) form is cutoff-dependent and ~10<sup>4</sup>× Maxwellian. The conductive heat flux is the third velocity moment, carried by the suprathermal tail (~40% of the κ = 2.5 distribution's energy sits above the Shoub v⁴ collisionless threshold), so it is non-local — the Fourier-law form itself fails, not merely its temperature input. The standard fluid-conductive budget therefore rests on a closure that does not exist in this regime; computing the true kinetic flux is the open transport problem of Edmonds 2026a §7. The −16.5 *λ*<sub>M</sub> the closed form returns at κ = 2.5 is an analytic continuation of a divergent integral, not a physical value.
...
- Edmonds, V. 2026b, *Open Transport*, 1, 20260011, doi:10.1515/ot-2026-0011 (convergence principle; SOL conductive-closure analysis §4.2.5)
...
|---|---|---|
| `kappa_conductivity_collapse.py` | `kappa_conductivity_collapse.{json,txt}` | **Closure non-existence (Fig. 9, §4.5.2).** Du 2013 Eq. 26 across κ: poles at κ = 2, 3, 4; −16.5 *λ*<sub>M</sub> at κ = 2.5; → 1 only for κ ≳ 5. Validation gate: the dominant heat-flux integrand (Du Eq. 25) reproduces its analytic Beta-function closed form to machine precision for κ > 4 (worst \|ratio−1\| = 7.8×10⁻⁴); Maxwellian carriers peak at 2.35 *v*<sub>th</sub> (integral 60); the κ = 2.5 conductivity integral diverges (cumulative ∝ *v*<sub>c</sub>³). The −16.5 at κ = 2.5 is an analytic continuation of the divergent integral, not a physical conductivity. Pure numpy+scipy; no atomic database. |
...
| `backreaction_bound.py` | `backreaction_bound.json` | Static-loop self-consistency check (superseded context): examined whether the temperature substitution would be multiplicative at the observed structure. The substitution is withdrawn in revision — there is no convergent Spitzer–Härm conductivity to substitute a temperature into (see `kappa_conductivity_collapse.py`). Retained for provenance. |
